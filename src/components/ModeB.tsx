...
import { runParallelAnalysis } from '../services/ai';
...
export default function ModeB() {
  ...
  const handleRun = async () => {
    ...
    try {
      const result = await runParallelAnalysis(
        t, file,
        ['highlights', 'risks', 'esg', 'competitors'],
  ...
  if (analysis) {
    return (
      <div className="flex-1 overflow-y-auto p-8">
        <AnalysisDashboard
          data={analysis}
          onReset={() => { setAnalysis(null); setFile(null); setTicker(''); }}
  ...
  const canRun = ticker.trim().length > 0 && !!file && !loading;
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#080a0f]/80 border border-slate-800 rounded-xl p-5">
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled={loading}
                className="w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/30 outline-none transition-all text-sm disabled:opacity-50"
              />
            </div>
          </div>
            ...
            {file ? (
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-500 mt-0.5">Annual reports, earnings releases, research reports (max 20 MB)</div>
                </div>
              </div>
            )}
          </div>
          ...
          <button
            onClick={handleRun}
            disabled={!canRun}
            className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold rounded-xl transition-all shadow-[0_0_15px_rgba(16,185,129,0.2)] hover:shadow-[0_0_25px_rgba(16,185,129,0.4)] text-sm tracking-wide"
          >
            {loading ? 'Running Parallel Analysis…' : 'Run Combined Analysis'}
          </button>
